...
func (a *autoCodeSqlite) GetDB(businessDB string) (data []response.Db, err error) {
	...
	if businessDB == "" {
		err = global.GvaDb.Raw(sql).Find(&databaseList).Error
	} else {
		err = global.GvaDbList[businessDB].Raw(sql).Find(&databaseList).Error
	}
...
func (a *autoCodeSqlite) GetTables(businessDB string, dbName string) (data []response.Table, err error) {
	...
	tabelNames := []string{}
	if businessDB == "" {
		err = global.GvaDb.Raw(sql).Find(&tabelNames).Error
	} else {
		err = global.GvaDbList[businessDB].Raw(sql).Find(&tabelNames).Error
	}
...
func (a *autoCodeSqlite) GetColumn(businessDB string, tableName string, dbName string) (data []response.Column, err error) {
	...
	if businessDB == "" {
		err = global.GvaDb.Raw(sql).Scan(&columnInfos).Error
	} else {
		err = global.GvaDbList[businessDB].Raw(sql).Scan(&columnInfos).Error
	}
