...
func (userService *UserService) SetUserAuthority(id uint, authorityId uint) (err error) {
	...
	err = global.GvaDb.Where("sys_authority_authority_id = ?", authorityId).Find(&authorityMenu).Error
	if err != nil {
		return err
	}
	...
	err = global.GvaDb.Preload("Parameters").Where("id in (?)", authorityMenuIDs).Find(&authorityMenus).Error
	if err != nil {
		return err
	}
	hasMenu := false
	...
	err = global.GvaDb.Model(&system.SysUser{}).Where("id = ?", id).Update("authority_id", authorityId).Error
	return err
}
...
func (userService *UserService) SetSelfSetting(req common.JSONMap, uid uint) error {
	return global.GvaDb.Model(&system.SysUser{}).Where("id = ?", uid).Update("origin_setting", req).Error
}
