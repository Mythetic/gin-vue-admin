...
func (baseMenuService *BaseMenuService) DeleteBaseMenu(id int) (err error) {
	err = global.GvaDb.First(&system.SysBaseMenu{}, "parent_id = ?", id).Error
	if err == nil {
		return errors.New("此菜单存在子菜单不可删除")
	}
	var menu system.SysBaseMenu
	err = global.GvaDb.First(&menu, id).Error
	if err != nil {
		return errors.New("记录不存在")
	}
	err = global.GvaDb.First(&system.SysAuthority{}, "default_router = ?", menu.Name).Error
	if err == nil {
