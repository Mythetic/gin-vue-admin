...
func (apiService *ApiService) SyncApi() (newApis, deleteApis, ignoreApis []system.SysApi, err error) {
	...
	err = global.GvaDb.Find(&ignores).Error
	if err != nil {
		return
	}
	...
	for i := range global.GvaRouters {
		ignoresFlag := false
		for j := range ignores {
			if ignores[j].Path == global.GvaRouters[i].Path && ignores[j].Method == global.GvaRouters[i].Method {
				ignoresFlag = true
			}
		}
		if !ignoresFlag {
			cacheApis = append(cacheApis, system.SysApi{
				Path:   global.GvaRouters[i].Path,
				Method: global.GvaRouters[i].Method,
			})
		}
	}
...
func (apiService *ApiService) IgnoreApi(ignoreApi system.SysIgnoreApi) (err error) {
	if ignoreApi.Flag {
		return global.GvaDb.Create(&ignoreApi).Error
	}
	return global.GvaDb.Unscoped().Delete(&ignoreApi, "path = ? AND method = ?", ignoreApi.Path, ignoreApi.Method).Error
}
...
func (apiService *ApiService) DeleteApi(api system.SysApi) (err error) {
	...
	err = global.GvaDb.First(&entity, "id = ?", api.ID).Error // 根据id查询api记录
...
func (apiService *ApiService) GetAllApis(authorityID uint) (apis []system.SysApi, err error) {
	...
	err = global.GvaDb.Order("id desc").Find(&apis).Error
	if parentAuthorityID == 0 || !global.GvaConfig.System.UseStrictAuth {
		return
	}
...
func (apiService *ApiService) GetApiById(id int) (api system.SysApi, err error) {
	err = global.GvaDb.First(&api, "id = ?", id).Error
	return
}
...
func (apiService *ApiService) UpdateApi(api system.SysApi) (err error) {
	...
	err = global.GvaDb.First(&oldA, "id = ?", api.ID).Error
	if oldA.Path != api.Path || oldA.Method != api.Method {
		var duplicateApi system.SysApi
		if ferr := global.GvaDb.First(&duplicateApi, "path = ? AND method = ?", api.Path, api.Method).Error; ferr != nil {
	...
	return global.GvaDb.Save(&api).Error
}
