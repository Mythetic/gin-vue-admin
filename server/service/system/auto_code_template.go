...
func (s *autoCodeTemplate) checkPackage(Pkg string, template string) (err error) {
	switch template {
	case "package":
		apiEnter := filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Server, "api", "v1", Pkg, "enter.go")
		_, err = os.Stat(apiEnter)
		if err != nil {
			return fmt.Errorf("package结构异常,缺少api/v1/%s/enter.go", Pkg)
		}
		serviceEnter := filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Server, "service", Pkg, "enter.go")
		_, err = os.Stat(serviceEnter)
		if err != nil {
			return fmt.Errorf("package结构异常,缺少service/%s/enter.go", Pkg)
		}
		routerEnter := filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Server, "router", Pkg, "enter.go")
		_, err = os.Stat(routerEnter)
		if err != nil {
			return fmt.Errorf("package结构异常,缺少router/%s/enter.go", Pkg)
		}
	case "plugin":
		pluginEnter := filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Server, "plugin", Pkg, "plugin.go")
...
func (s *autoCodeTemplate) Create(ctx context.Context, info request.AutoCode) error {
	...
	err := global.GvaDb.WithContext(ctx).Where("package_name = ?", info.Package).First(&autoPkg).Error
	if err != nil {
	...
	if info.AutoCreateApiToSql && !info.OnlyTemplate {
		apis := info.Apis()
		err := global.GvaDb.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
			for _, v := range apis {
	...
	if info.AutoCreateMenuToSql {
		...
		var id uint
		err := global.GvaDb.WithContext(ctx).First(&entity, "name = ?", info.Abbreviation).Error
		if err == nil {
			id = entity.ID
		} else {
			...
			err = global.GvaDb.WithContext(ctx).Create(&entity).Error
			id = entity.ID
			if err != nil {
...
func (s *autoCodeTemplate) Preview(ctx context.Context, info request.AutoCode) (map[string]string, error) {
	...
	err := global.GvaDb.WithContext(ctx).Where("package_name = ?", info.Package).First(&entity).Error
	if err != nil {
	...
	for key, writer := range codes {
		if len(key) > len(global.GvaConfig.AutoCode.Root) {
			key, _ = filepath.Rel(global.GvaConfig.AutoCode.Root, key)
		}
		// 获取key的后缀 取消.
...
func (s *autoCodeTemplate) AddFunc(info request.AutoFunc) error {
	...
	err := global.GvaDb.First(&autoPkg, "package_name = ?", info.Package).Error
	if err != nil {
		return err
	}
...
func (s *autoCodeTemplate) getTemplateStr(t string, info request.AutoFunc) (string, error) {
	tempPath := filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Server, "resource", "function", t+".tpl")
...
func (s *autoCodeTemplate) addTemplateToAst(t string, info request.AutoFunc) error {
	tPath := filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Server, "router", info.Package, info.HumpPackageName+".go")
	...
	if info.IsPlugin {
		tPath = filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Server, "plugin", info.Package, "router", info.HumpPackageName+".go")
...
func (s *autoCodeTemplate) addTemplateToFile(t string, info request.AutoFunc) error {
	...
	switch t {
	case "api.go":
		target = filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Server, "api", "v1", info.Package, info.HumpPackageName+".go")
	case "server.go":
		target = filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Server, "service", info.Package, info.HumpPackageName+".go")
	case "api.js":
		target = filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Web, "api", info.Package, info.PackageName+".js")
	}
	if info.IsPlugin {
		switch t {
		case "api.go":
			target = filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Server, "plugin", info.Package, "api", info.HumpPackageName+".go")
		case "server.go":
			target = filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Server, "plugin", info.Package, "service", info.HumpPackageName+".go")
		case "api.js":
			target = filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Web, "plugin", info.Package, "api", info.PackageName+".js")
		}
	}
